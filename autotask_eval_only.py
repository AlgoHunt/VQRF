...
def run_exp(env,  config, datadir, expname, basedir):
    ...
    base_cmd = ['python', 'run_load_compressed.py',  '--config', auto_config_path, '--eval_ssim','--eval_lpips_vgg',
            '--eval_lpips_alex' , '--render_test',  '--fully_vq --render_only', 
...
txt_file = os.path.join(datasetting["basedir"], f'merge_compressed.txt')
with open(txt_file, 'w') as f:
    f.writelines(table.get_string())

csv_file = os.path.join(datasetting["basedir"], f'merge_compressed.csv')
with open(csv_file, 'w', newline='') as f:
    f.writelines(table.get_csv_string())
print('=================================================================================================') 
print('VQ-DVGO (loaded from extreme saving):')
print(table)
